...
import { EPISODES, LOGO_URL } from '../constants';

interface HeroProps {
  onWatchEpisode: (ep: Episode) => void;
}

const Hero: React.FC<HeroProps> = ({ onWatchEpisode }) => {
    ...
    <section className="relative pt-12 pb-24 lg:pt-20 lg:pb-32 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center mb-16 text-center">
          ...
          <h1 className="text-6xl md:text-9xl font-bold text-slate-900 tracking-tighter leading-none mb-4">
        ...
        {/* Featured Video Card - More Prominent */}
        <div className="relative group">
          ...
          <div className="relative bg-white rounded-[3rem] overflow-hidden shadow-2xl border border-white flex flex-col lg:flex-row min-h-[500px]">
            ...
            <div className="w-full lg:w-3/5 relative aspect-video lg:aspect-auto overflow-hidden">
              ...
              {/* Play Button Overlay */}
              <button 
                onClick={() => onWatchEpisode(featured)}
            ...
            <div className="w-full lg:w-2/5 p-8 md:p-12 flex flex-col justify-between bg-white">
              <div className="space-y-8">
                <div>
                  <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4 leading-tight group-hover:text-cyan-600 transition-colors">
                    {featured.title}
                  </h2>
                ...
                {featured.guest && (
                  <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-4">
                    <div className="flex items-center gap-4">
                      <img 
                        src={featured.guest.photo} 
                        alt={featured.guest.name} 
                        className="w-16 h-16 rounded-2xl object-cover shadow-md border-2 border-white"
                      />
                      <div>
                        <p className="text-xs font-bold text-cyan-600 uppercase tracking-widest mb-1">Invitada Especial</p>
                        <p className="text-lg font-bold text-slate-900">{featured.guest.name}</p>
                        <p className="text-sm text-slate-500">{featured.guest.role}</p>
                      </div>
                    </div>
